...
public class SecureRandom extends java.util.Random {
    ...
    private void getDefaultPRNG(boolean setSeed, byte[] seed) {
        String prng = getPrngAlgorithm();//获取是否有SecureRandom的继承类，并且组合了SecureRandomSpi类（也就是自己可以自定义生成随机数的实现类）
        if (prng == null) {
            // bummer, get the SUN implementation 如果不存在，则使用sun自带的实现类
            prng = "SHA1PRNG";
            this.secureRandomSpi = new sun.security.provider.SecureRandom();
            this.provider = Providers.getSunProvider();
            if (setSeed) {
                this.secureRandomSpi.engineSetSeed(seed);//设置随机数种子
            }
        } else {
            try {
